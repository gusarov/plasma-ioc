...
namespace TurboFac
{
 ...
 parameter.IsOptional,
		...
		public object DefaultFactory(Type type)
		{
			...
			TurboContainer.ValidateImplementationType(type);
		...
		{
			// validate
			if (!TurboContainer.IsValidImplementationType(parameterType))
			{
				if (isOptional)
				{
	...
	class ReflectionMining : Mining
	{
		...
		public ReflectionMining(TurboContainer provider)
		{
			_provider = provider;
		}
